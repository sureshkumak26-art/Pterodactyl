...
async function request(method, path, data, params) {
  try {
    ...
    if (status === 404) {
      throw new Error(`HTTP 404: ${method} ${path} was not found. Check PTERO_URL and verify that the supplied ID exists and belongs to the selected parent.`);
    }
    throw new Error(`HTTP ${status || 'ERR'} ${method} ${path}\n${e.response?.data?.message || e.message || 'Unknown Pterodactyl API error'}`);
  }
}

// Validate all resources required before creating a server.
// This function intentionally calls request() directly so it does not depend
// on module-exported functions that are initialized later.
async function getCreateServerResources({ user, node, nest, egg, allocation }) {
  const [userData, nodeData, eggData, allocationData] = await Promise.all([
  ...
  const allocations = allocationData.data || [];
  const selectedAllocation = allocations.find(
    x => Number(x.attributes?.id) === Number(allocation)
  );

  if (!selectedAllocation) {
    throw new Error(
      `Allocation ${allocation} was not found on node ${node}. Run /allocations node:${node} and choose an Available allocation.`
    );
  }

  if (selectedAllocation.attributes.assigned) {
    throw new Error(
      `Allocation ${allocation} is already assigned. Run /allocations node:${node} and choose an Available allocation.`
    );
  }
  ...
  return {
    ...
    allocation: selectedAllocation.attributes
  };
}

module.exports = {
  getUsers: (filter) => request('GET', '/users', undefined, {
    'filter[email]': filter || undefined,
    per_page: 100
  }),
  ...
  getNests: () => request('GET', '/nests', undefined, {
    include: 'eggs',
    per_page: 100
  }),
  getNest: (id) => request('GET', `/nests/${id}`, undefined, { include: 'eggs' }),
  getEgg: (nestId, eggId) => request(
    'GET',
