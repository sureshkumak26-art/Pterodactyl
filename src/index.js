...
client.on('interactionCreate', async (i) => {
  ...
  try {
    ...
    if (c === 'help') return i.reply({ embeds: [embed('☁️ Pterodactyl Bot Help', [
      ...
      '**🖥️ Servers**','`/create-server` — Create a server','`/server` — View a server','`/servers` — List servers','`/rename-server` — Rename a server','`/delete-server` — Delete a server','`/suspend` — Suspend a server','`/unsuspend` — Unsuspend a server','`/start` — Start a server','`/stop` — Stop a server','`/restart` — Restart a server','',
    ...
    if (c === 'create-server') {
      ...
      const name=i.options.getString('name'), memory=i.options.getInteger('memory'), disk=i.options.getInteger('disk'), cpu=i.options.getInteger('cpu'), backups=i.options.getInteger('backups')||0;
